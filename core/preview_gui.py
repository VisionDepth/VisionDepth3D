...
def open_3d_preview_window(
    ...
    def _extract_shift_map(shift_meta):
        if isinstance(shift_meta, dict):
            shift_map = shift_meta.get("shift_map")
            if isinstance(shift_map, torch.Tensor):
                return shift_map.detach().cpu()
            return shift_map
        return None

    def _apply_dynamic_floating_window_preview(left_bgr, right_bgr, shift_meta, per_eye_width):
        if not (use_floating_window.get() and use_subject_tracking.get()):
            return left_bgr, right_bgr

        if not isinstance(shift_meta, dict):
            return left_bgr, right_bgr

        if not hasattr(render_3d, "dfw_last_side"):
            render_3d.dfw_last_side = "left"
            render_3d.dfw_last_width = 0
        ...
        if parallax_mag < render_3d.DFW_MIN_PARALLAX:
            target_width = 0
        else:
            depth_delta = abs(subject_depth_val - 0.5)
            parallax_delta = (
                render_3d.DFW_PARALLAX_WEIGHT * parallax_mag
                + render_3d.DFW_DEPTH_WEIGHT * depth_delta
            )
            parallax_delta = min(parallax_delta, 0.12)

            target_width = int(per_eye_width * parallax_delta)
            max_bar_px = int(per_eye_width * render_3d.DFW_MAX_BAR_FRAC)
            target_width = max(0, min(target_width, max_bar_px))
            render_3d.dfw_last_side = "left" if zero_parallax_offset > 0.0 else "right"

        render_3d.dfw_last_width = int(
            render_3d.DFW_WIDTH_EASE * getattr(render_3d, "dfw_last_width", 0)
            + (1.0 - render_3d.DFW_WIDTH_EASE) * target_width
        )

        if render_3d.dfw_last_width > 1:
            if render_3d.DFW_USE_FADE:
                left_bgr = render_3d.apply_side_mask(
                    left_bgr,
                    side=render_3d.dfw_last_side,
                    width=render_3d.dfw_last_width,
                    fade=True,
                    solid_black=False,
                )
                right_bgr = render_3d.apply_side_mask(
                    right_bgr,
                    side=render_3d.dfw_last_side,
                    width=render_3d.dfw_last_width,
                    fade=True,
    ...
    def update_preview_now():
        ...
        try:
            render_3d.reset_render_state()
        except Exception:
            # Fallback for older render_3d copies
            try:
        ...
        if depth is None:
            messagebox.showerror(
                "Frame Error",
                "Unable to grab frame from DEPTH video at this index. (Depth is shorter or unreadable at this position.)"
            )
            return

        h, w = frame.shape[:2]
        frame_tensor = F.interpolate(
            render_3d.frame_to_tensor(frame).unsqueeze(0),
            size=(h, w),
            mode='bilinear',
            align_corners=False
        ).squeeze(0)
        depth_tensor = F.interpolate(
            render_3d.depth_to_tensor(depth).unsqueeze(0),
            size=(h, w),
            mode='bilinear',
            align_corners=False
        ).squeeze(0)
        ...
        try:
            blur_val = int(blur_ksize.get())
            feather_val = float(feather_strength.get())
            ...
            dynamic_convergence_enabled = bool(enable_dynamic_convergence.get())
        except ValueError:
            messagebox.showwarning("Input Error", "One or more numeric settings are invalid.")
            return

        local_temporal = render_3d.TemporalDepthFilter(alpha=0.5)
        depth_tensor = local_temporal.smooth(depth_tensor)
        depth_tensor = render_3d.depth_ema_norm.normalize(depth_tensor)

        fg_val = float(fg_shift.get())
        mg_val = float(mg_shift.get())
        bg_val = float(bg_shift.get())

        smoother = render_3d.ShiftSmoother(alpha=0.15)
        fg_val, mg_val, bg_val = smoother.smooth(fg_val, mg_val, bg_val)

        try:
            dyn_scale = render_3d.compute_dynamic_parallax_scale(
                depth_tensor,
                min_scale=0.90,
                max_scale=1.15,
            )
        ...
        try:
            left_tensor, right_tensor, shift_meta = render_3d.pixel_shift_cuda(
                frame_tensor,
                depth_tensor,
                w,
                h,
            ...
            messagebox.showerror("Preview Error", f"3D preview generation failed:\n{exc}")
            return

        need_dof = dof_strength_val > 0.0
        need_color = (
            abs(float(saturation.get()) - 1.0) > 1e-6
            or abs(float(contrast.get()) - 1.0) > 1e-6
            or abs(float(brightness.get())) > 1e-6
        )

        if need_dof or need_color:
            left_eye = left_tensor
            right_eye = right_tensor
            eye_h, eye_w = left_eye.shape[1], left_eye.shape[2]
            ...
            if need_dof:
                focal_depth = render_3d.estimate_subject_depth(depth_tensor)
                left_eye = render_3d.apply_dof_cuda(
                    left_eye,
                    depth_for_eye,
                    focal_depth,
                    max_sigma=dof_strength_val,
                    focus_width=0.35,
                )
                right_eye = render_3d.apply_dof_cuda(
                    right_eye,
                    depth_for_eye,
                    focal_depth,
                    max_sigma=dof_strength_val,
            ...
            if need_color:
                left_eye = render_3d.apply_color_grade(
                    left_eye,
                    saturation=saturation.get(),
                    contrast=contrast.get(),
                    brightness=brightness.get(),
                )
                right_eye = render_3d.apply_color_grade(
                    right_eye,
                    saturation=saturation.get(),
                    contrast=contrast.get(),
                    brightness=brightness.get(),
        ...
        left_frame = render_3d.tensor_to_frame(left_eye)
        right_frame = render_3d.tensor_to_frame(right_eye)

        left_out = render_3d.apply_sharpening(left_frame, sharpness_val)
        right_out = render_3d.apply_sharpening(right_frame, sharpness_val)

        left_out, right_out = _apply_dynamic_floating_window_preview(left_out, right_out, shift_meta, w)

        shift_map = _extract_shift_map(shift_meta)
        preview_mode = preview_type_var.get()

        if preview_mode in ("HSBS", "Half-SBS"):
            preview_img = render_3d.format_3d_output(left_out, right_out, "Half-SBS")
        elif preview_mode == "Passive Interlaced":
            preview_img = render_3d.format_3d_output(left_out, right_out, "Passive Interlaced")
        else:
        ...
        if preview_img is not None:
            if show_convergence_guides.get():
                preview_img = draw_convergence_guides(preview_img, left_out.shape[1], left_out.shape[0])
