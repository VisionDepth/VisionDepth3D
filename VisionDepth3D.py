...
from tkinter import ttk, filedialog, Label, Button, OptionMenu, StringVar, BooleanVar, Entry, messagebox
...
def format_3d_output(left_frame, right_frame, output_format):
    ...
    if output_format == "Full-SBS":
        ...
        left_resized = cv2.resize(left_frame, (width // 2, height), interpolation=cv2.INTER_LANCZOS4)
        right_resized = cv2.resize(right_frame, (width // 2, height), interpolation=cv2.INTER_LANCZOS4)
        ...
        left_resized = cv2.resize(left_frame, (width, height // 2), interpolation=cv2.INTER_LANCZOS4)
        right_resized = cv2.resize(right_frame, (width, height // 2), interpolation=cv2.INTER_LANCZOS4)
        return np.vstack((left_resized, right_resized))  # Half-OU (1920x1080)

    elif output_format == "VR180":
        left_resized = cv2.resize(left_frame, (1440, 1600), interpolation=cv2.INTER_LANCZOS4)
        right_resized = cv2.resize(right_frame, (1440, 1600), interpolation=cv2.INTER_LANCZOS4)
        return np.vstack((left_resized, right_resized))  # VR180 (2880x1600)

    elif output_format == "VR360":
        left_resized = cv2.resize(left_frame, (1920, 960), interpolation=cv2.INTER_LANCZOS4)
        right_resized = cv2.resize(right_frame, (1920, 960), interpolation=cv2.INTER_LANCZOS4)
...
def correct_convergence_shift(left_frame, right_frame, depth_map, session, input_name, output_name, bg_threshold=3.0):
    ...
    # ✅ Normalize depth map
    depth_gpu = cv2.cuda.normalize(depth_gpu, alpha=0.1, beta=1.0, norm_type=cv2.NORM_MINMAX, dtype=cv2.CV_32F)
    ...
    warp_params = session.run([output_name], {input_name: warp_input})[0].reshape(3, 3).astype(np.float32)
    ...
    # ✅ Apply warping
    corrected_left_gpu = cv2.cuda.warpPerspective(left_gpu, warp_params, (w, h), flags=cv2.INTER_CUBIC)
    corrected_right_gpu = cv2.cuda.warpPerspective(right_gpu, warp_params, (w, h), flags=cv2.INTER_CUBIC)
    ...
    inpainted_left = cv2.inpaint(corrected_left, mask_left, inpaintRadius=15, flags=cv2.INPAINT_NS)
    inpainted_right = cv2.inpaint(corrected_right, mask_right, inpaintRadius=15, flags=cv2.INPAINT_NS)
...
def render_sbs_3d(input_video, depth_video, output_video, codec, fps, width, height, fg_shift, mg_shift, bg_shift,
    ...
    out = cv2.VideoWriter(output_video, codec_map[codec], fps, (width * 2 if output_format == "Full-SBS" else width, height))
    ...
    for frame_idx in range(total_frames):
        ...
        depth_gpu = cv2.cuda.normalize(depth_gpu, alpha=0, beta=1, norm_type=cv2.NORM_MINMAX, dtype=cv2.CV_32F)
        ...
        depth_cpu_resized = cv2.resize(depth_cpu, (width, height), interpolation=cv2.INTER_AREA)
        ...
        x_coords = np.tile(np.arange(width), (height, 1))  # Create a 2D array of x-coordinates

        new_x_left = np.clip(x_coords + shift_vals_fg + shift_vals_mg + shift_vals_bg, 0, width - 1)
        new_x_right = np.clip(x_coords - shift_vals_fg - shift_vals_mg - shift_vals_bg, 0, width - 1)
        ...
        left_frame_gpu = cv2.cuda.remap(frame_gpu, map_x_left_gpu, map_y_gpu, interpolation=cv2.INTER_CUBIC)
        right_frame_gpu = cv2.cuda.remap(frame_gpu, map_x_right_gpu, map_y_gpu, interpolation=cv2.INTER_CUBIC)
...
def render_ou_3d(input_video, depth_video, output_video, codec, fps, width, height,
    ...
    for frame_idx in range(total_frames):
        ...
        if progress:
            ...
            progress.update()
        if progress_label:
            progress_label.config(text=f"{percentage:.2f}% | FPS: {avg_fps:.2f} | Elapsed: {time.strftime('%M:%S', time.gmtime(elapsed_time))}")
        prev_time = curr_time
        ...
        map_y = np.repeat(np.arange(height).reshape(-1, 1), width, axis=1).astype(np.float32)
        new_y_top = np.clip(map_y + shift_vals_fg + shift_vals_mg + shift_vals_bg, 0, height - 1)
        new_y_bottom = np.clip(map_y - shift_vals_fg - shift_vals_mg - shift_vals_bg, 0, height - 1)
        ...
        top_frame_gpu = cv2.cuda.remap(frame_gpu, map_x_gpu, map_y_top_gpu, interpolation=cv2.INTER_CUBIC)
        bottom_frame_gpu = cv2.cuda.remap(frame_gpu, map_x_gpu, map_y_bottom_gpu, interpolation=cv2.INTER_CUBIC)
...
local_model_dir = os.path.expanduser("~/.cache/huggingface/models/")  # Correct local path
...
def ensure_model_downloaded(checkpoint):
    ...
    local_path = os.path.join(local_model_dir, checkpoint.replace("/", "_"))  # Convert to valid folder name

    if not os.path.exists(local_path):
        print(f"📥 Downloading model: {checkpoint} ... This may take a few minutes.")
        try:
            # ✅ Download model & processor to local storage
            AutoModelForDepthEstimation.from_pretrained(checkpoint, cache_dir=local_path)
...
def update_pipeline(*args):
    ...
    try:
        ...
        processor = AutoProcessor.from_pretrained(checkpoint, cache_dir=local_model_path)
        model = AutoModelForDepthEstimation.from_pretrained(checkpoint, cache_dir=local_model_path).to(device)

        # ✅ Load the depth-estimation pipeline
        pipe = pipeline("depth-estimation", model=model, device=device, image_processor=processor)

        # ✅ Update status label to show only the model name
        status_label.config(text=f"✅ Model loaded: {selected_checkpoint} (Running on {device.upper()})")
...
def process_image_folder():
    ...
    threading.Thread(target=process_images_in_folder, args=(folder_path,), daemon=True).start()
...
def process_images_in_folder(folder_path):
    ...
    if not image_files:
        root.after(10, lambda: status_label.config(text="⚠️ No image files found in the selected folder."))
        return
    ...
    for i, image_file in enumerate(image_files, start=1):
        ...
        root.after(10, lambda i=i, fps=fps, eta=eta: update_progress(i, total_images, fps, eta))
...
def process_videos_in_folder(folder_path):
    ...
    total_frames_all = sum(cv2.VideoCapture(os.path.join(folder_path, f)).get(cv2.CAP_PROP_FRAME_COUNT) for f in video_files)
    ...
    for video_file in video_files:
        ...
        frames_processed_all += process_video2(video_path, total_frames_all, frames_processed_all, BATCH_SIZE)  # ✅ Pass BATCH_SIZE
    ...
    while True:
        ...
        if len(frames_batch) == BATCH_SIZE or frame_count == total_frames:
            ...
            for i, prediction in enumerate(predictions):
                ...
                if save_frames:
                    frame_filename = os.path.join(frame_output_dir, f"frame_{frame_count - len(predictions) + i + 1}.png")
...
def process_video3():
    ...
    width_value, height_value = int(width.get()), int(height.get())  # ✅ Ensure integer values
    ...
    video = cv2.VideoWriter(output_file.get(), codec, fps_value, (width_value, height_value), isColor=True)
    
    if not video.isOpened():
        messagebox.showerror("Error", "Failed to open video writer. Check codec and output path!")
        return
    ...
    def process_frame(frame_path):
        ...
        frame_resized = cv2.resize(frame, (width_value, height_value), interpolation=cv2.INTER_LINEAR)

        # ✅ Auto-detect Grayscale and Convert to 3-Channel if needed
        if len(frame_resized.shape) == 2:  # Single-channel grayscale
            frame_resized = cv2.cvtColor(frame_resized, cv2.COLOR_GRAY2BGR)  # Convert grayscale to RGB
    ...
    with ThreadPoolExecutor(max_workers=4) as executor:
        for index, frame in enumerate(executor.map(process_frame, frames), start=1):
            if frame is not None:
                video.write(frame)
                root.after(10, lambda: vdstitch_progress.config(value=index))  # ✅ Update VDStitch progress bar

    video.release()
    
    # ✅ Ensure progress reaches 100% at the end
    root.after(10, lambda: vdstitch_progress.config(value=vdstitch_progress["maximum"]))  # ✅ VDStitch bar updates
    root.after(10, lambda: messagebox.showinfo("Success", "Video processing completed!"))
...
save_frames_checkbox = tk.Checkbutton(sidebar, text=" Save Frames", variable=save_frames_var, bg="#1c1c1c", fg="white")
...
status_label = tk.Label(sidebar, text="🔋 Ready", bg="#1c1c1c", fg="white", width=30, wraplength=200)
status_label.pack(pady=5)

depth_map_label_depth = tk.Label(sidebar, text="Previous Depth Map: None", justify="left", wraplength=200)
...
tk.Button(button_frame, text="🖼️ Process Image", command=open_image,
          width=25, bg="#4a4a4a", fg="white").pack(pady=2)
tk.Button(button_frame, text="🖼️ Process Image Folder", command=process_image_folder,
          width=25, bg="#4a4a4a", fg="white").pack(pady=2)
tk.Button(button_frame, text="🎥 Process Video", command=open_video,
          width=25, bg="#4a4a4a", fg="white").pack(pady=2)
tk.Button(button_frame, text="📂 Select Video Folder", command=process_video_folder,
          width=25, bg="#4a4a4a", fg="white").pack(pady=2)
...
tk.Label(VDStitch, text="📂 Select Frames Folder:", bg="#1c1c1c", fg="white").pack(anchor="w", padx=10, pady=5)
tk.Entry(VDStitch, textvariable=frames_folder, width=50, 
         bg="#2b2b2b", fg="white", insertbackground="white", relief="flat").pack(padx=10, pady=2)
tk.Button(VDStitch, text="Browse", command=select_frames_folder, 
          bg="#4a4a4a", fg="white", relief="flat").pack(pady=2)

tk.Label(VDStitch, text="🎥 Select Output Video File:", bg="#1c1c1c", fg="white").pack(anchor="w", padx=10, pady=5)
tk.Entry(VDStitch, textvariable=output_file, width=50, 
         bg="#2b2b2b", fg="white", insertbackground="white", relief="flat").pack(padx=10, pady=2)
tk.Button(VDStitch, text="Save As", command=select_output_file, 
          bg="#4a4a4a", fg="white", relief="flat").pack(pady=2)

tk.Label(VDStitch, text="🖼️ Resolution (Width x Height):", bg="#1c1c1c", fg="white").pack(anchor="w", padx=10, pady=5)
frame_res = tk.Frame(VDStitch, bg="#1c1c1c")
frame_res.pack()
tk.Entry(frame_res, textvariable=width, width=10, 
         bg="#2b2b2b", fg="white", insertbackground="white", relief="flat").pack(side="left", padx=5)
tk.Label(frame_res, text="x", bg="#1c1c1c", fg="white").pack(side="left")
tk.Entry(frame_res, textvariable=height, width=10, 
         bg="#2b2b2b", fg="white", insertbackground="white", relief="flat").pack(side="left", padx=5)

tk.Label(VDStitch, text="🎞 Select Codec:", bg="#1c1c1c", fg="white").pack(anchor="w", padx=10, pady=5)
codec_menu = ttk.Combobox(VDStitch, textvariable=selected_codec, values=list(CODECS.keys()), state="readonly")
codec_menu.pack(padx=10, pady=2)

tk.Label(VDStitch, text="⏱ Frame Rate (FPS):", bg="#1c1c1c", fg="white").pack(anchor="w", padx=10, pady=5)
tk.Entry(VDStitch, textvariable=fps, width=10, 
         bg="#2b2b2b", fg="white", insertbackground="white", relief="flat").pack(padx=10, pady=2)

# Processing Button with Dark Theme
process_btn = tk.Button(VDStitch, text="▶ Start Processing", command=start_processing, 
